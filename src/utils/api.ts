import { APIInterface, APIOptions, Category, Item, ItemsAPIResponse, SearchSuggestion } from '../types';
...
export const SAMPLE_ITEMS: Item[] = [
  {
    id: '1',
    title: '빈티지 레트로 라디오',
    description: '1960년대 스타일의 아날로그 라디오입니다. 완벽한 작동 상태이며 수집가들에게 인기가 많습니다.',
  ...
  },
  {
    id: '2',
    title: '명품 가죽 백팩',
    description: '이탈리아산 프리미엄 가죽으로 제작된 백팩입니다. 실용적이면서도 세련된 디자인입니다.',
  ...
  },
  {
    id: '3',
    title: '골동품 청화백자',
    description: '조선시대 청화백자 항아리입니다. 보존 상태가 매우 양호하며 골동품 수집가들에게 추천합니다.',
  ...
  },
  {
    id: '4',
    title: '스마트폰 거치대',
    description: '다양한 각도로 조절 가능한 스마트폰 거치대입니다. 홈오피스에 최적화되어 있습니다.',
...
export class MockAPI implements APIInterface {
  ...
  async getItemsByCategory(categoryId: string, options: APIOptions = {}): Promise<ItemsAPIResponse> {
    await this.simulateDelay();
    ...
    const categoryItems = SAMPLE_ITEMS.filter(item => item.category === category.name);
  ...
  async getSearchSuggestions(query: string): Promise<SearchSuggestion[]> {
    ...
    tagSuggestions.forEach(tag => {
      const count = SAMPLE_ITEMS.filter(item => item.tags.includes(tag)).length;
      suggestions.push({
        term: tag,
        count
      });
