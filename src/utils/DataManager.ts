...
export class DataManager {
  ...
  getFilteredItems(): Item[] {
    ...
    if (minPrice > 0 || maxPrice < 10000000) {
      filteredItems = filteredItems.filter(
        (item) => item.currentBid >= minPrice && item.currentBid <= maxPrice,
      );
    }
  ...
  getFeaturedItems(limit: number = 4): Item[] {
    return this.state.items.filter((item) => item.featured).slice(0, limit);
  }
  ...
  getStats() {
    ...
    const totalBids = this.state.items.reduce(
