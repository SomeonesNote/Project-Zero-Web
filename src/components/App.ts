...
export class App {
  ...
  private signUpComponent: SignUp | null = null;
  ...
  private async setupData(): Promise<void> {
    try {
      // 병렬로 데이터 로드
      const [featuredResponse, endingSoonResponse, categoriesResponse] =
        await Promise.all([
  ...
  private initializeComponents(): void {
    ...
    // 메인 검색바 컴포넌트
    const mainSearchContainer = document.querySelector(
      ".main-search",
    ) as HTMLElement;
    ...
    if (categoryContainer) {
      this.components.categoryTags = new CategoryTags(categoryContainer, {
        ...
        onCategoryChange: (selectedCategories: string[]) => {
          ...
          } as CustomEvent<CategoryChangeEvent>);
        },
      });
    }
      ...
      this.components.pagination = new Pagination(paginationContainer, {
        ...
        onPageChange: (page: number) => {
          this.handlePageChange({
            detail: { page },
          } as CustomEvent<PageChangeEvent>);
        },
      });
    }
  }
    ...
    try {
      ...
      if (response.length > 0) {
        const container = document.querySelector(
          ".featured-items .items-grid",
        ) as HTMLElement;
        if (container) {
          this.components.featuredGrid = new ItemGrid(container, {
            container,
            items: response,
            onItemClick: (data: { item: Item }) =>
              this.handleItemClick({
                detail: data,
              } as CustomEvent<ItemClickEvent>),
            onBidClick: (data: { item: Item }) =>
              this.handleBidClick({
                detail: data,
              } as CustomEvent<ItemClickEvent>),
          });
  ...
  private showSignUpPage(): void {
    ...
    // SignUp 컴포넌트 렌더링
    signupContainer.innerHTML = this.signUpComponent.render();
    this.signUpComponent.mount();
    
    (signupContainer as HTMLElement).style.display = "block";
  }
  ...
  private showMainPage(): void {
    ...
    if (signupContainer) {
      (signupContainer as HTMLElement).style.display = "none";
    }
  ...
  private async handleNavigation(
    event: CustomEvent<NavigationEvent>,
  ): Promise<void> {
  ...
  private async handleHeaderSearch(
    event: CustomEvent<SearchEvent>,
  ): Promise<void> {
  ...
  private async handleHeaderSearchSubmit(
    event: CustomEvent<SearchEvent>,
  ): Promise<void> {
    const { term } = event.detail;
  ...
  private async handleCategoryChange(
    event: CustomEvent<CategoryChangeEvent>,
  ): Promise<void> {
  ...
  private async handleItemClick(
    event: CustomEvent<ItemClickEvent>,
  ): Promise<void> {
  ...
  private async handlePageChange(
    event: CustomEvent<PageChangeEvent>,
  ): Promise<void> {
